    ...
    if (!$locked) {
        ...
        if ($check->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                ...
                registrar_log($conn, $_SESSION['usuario_id'], 'editar', 'item_vistoria', (int) $id, $nome_item);
        ...
        <div class="cadastro-area">
            ...
            <form method="POST" class="form-cadastro">
                ...
                <div class="form-grupo">
                    ...
                    <input type="text" name="nome_item" value="<?= htmlspecialchars($_POST['nome_item'] ?? $item['nome_item'] ?? '') ?>" required>
                </div>

                <div class="form-grupo">
                    <label>Descrição:</label>
                    <textarea name="descricao" rows="3"><?= htmlspecialchars($_POST['descricao'] ?? $item['descricao'] ?? '') ?></textarea>
                </div>

                <div class="form-grupo checkbox">
                    <label class="checkbox-container">
                        <input type="checkbox" name="ativo" <?= (isset($_POST['ativo']) && $_SERVER['REQUEST_METHOD'] === 'POST' ? true : (bool)$item['ativo']) ? 'checked' : '' ?>>
